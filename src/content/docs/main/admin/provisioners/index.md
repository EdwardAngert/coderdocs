...
description: 'By default, the Coder server runs'
version: main
audience: admin
---
...
[built-in provisioner daemons](../../reference/cli/server#--provisioner-daemons),
...
- **Isolate APIs:** Deploy provisioners in isolated environments (on-prem, AWS,
  ...
  server. See
  [Provider Authentication](../../admin/templates/extending-templates/provider-authentication)
  for more details.
...
- **Reduce server load**: External provisioners reduce load and build queue
  ...
  [Scaling Coder](../../admin/infrastructure/index#scale-tests) for more
  details.

Each provisioner runs a single
[concurrent workspace build](../../admin/infrastructure/scale-testing#control-plane-provisionerd).
For example, running 30 provisioner containers will allow 30 users to start
workspaces at the same time.

Provisioners are started with the
[`coder provisioner start`](../../reference/cli/provisioner_start) command in
...
You can use the dashboard, CLI, or API to [manage provisioners](./manage-provisioner-jobs).
...
global PSK, set a
[provisioner daemon pre-shared key (PSK)](../../reference/cli/server#--provisioner-daemon-psk)
on the Coder server.
...
![template tags](%images/%images/./../images/admin/provisioner-tags.png)
...
[workspace tags documentation](../../admin/templates/extending-templates/workspace-tags)
...
[`-tag=scope=<scope>`](../../reference/cli/provisioner_start#-t---tag) when
starting the provisioner daemon. Only users with at least the
[Template Admin](../users/index#roles) role or higher may create
organization-scoped provisioner daemons.

There are two exceptions:

- [Built-in provisioners](../../reference/cli/server#--provisioner-daemons) are
  always organization-scoped.
- External provisioners started using a
  [pre-shared key (PSK)](../../reference/cli/provisioner_start#--psk) are always
  organization-scoped.
...
[flag or environment variable](../../reference/cli/server#--provisioner-daemons).
...
- [Manage Provisioners](./manage-provisioner-jobs)
